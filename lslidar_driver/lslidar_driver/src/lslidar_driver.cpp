...
namespace lslidar_driver {
    ...
    bool lslidarDriver::loadParameters() {
        ...
        pnh.param<bool>("use_gps_ts", use_gps_ts, true);
        pnh.param<bool>("use_ros_time", use_ros_time, false);
        ...
        ROS_INFO("use gps time: %d, use ros time: %d", use_gps_ts, use_ros_time);
        return true;
    }
        ...
        if (pcl_type) {
            ...
            if (sweep_data->points.size() > 0) {
                for (j = 0; j < sweep_data->points.size(); ++j) {
                    ...
                    if ((sweep_data->points[j].azimuth > angle_disable_min) or
